...
export default function NavBar() {
  ...
  const openMobile = useCallback(() => { setMobileOpen(true); document.body.style.overflow = 'hidden' }, [])
  const closeMobile = useCallback(() => { setMobileOpen(false); setMobileSub(null); document.body.style.overflow = '' }, [])

  useEffect(() => { return () => { document.body.style.overflow = '' } }, [])

  useEffect(() => {
    if (activeDropdown === null) return
    const h = (e: MouseEvent) => { if (navRef.current && !navRef.current.contains(e.target as Node)) setActiveDropdown(null) }
    document.addEventListener('click', h)
    return () => document.removeEventListener('click', h)
  }, [activeDropdown])

  const toggleDropdown = (i: number) => setActiveDropdown(activeDropdown === i ? null : i)
  const openSub = (i: number) => { setMobileSub(i); setMobileSubAnim('in') }
  const closeSub = () => {
    if (mobileSubAnim === 'out') return
    setMobileSubAnim('out')
    setTimeout(() => { setMobileSub(null); setMobileSubAnim(null) }, 300)
  }
  ...
  return (
    <>
      ...
      <nav ref={navRef} className="hidden md:flex items-center gap-0.5">
        {menuItems.map((item, i) => (
          <div key={item.label} className="relative">
            {item.href ? (
              <Link href={item.href}
                className="flex items-center gap-1 px-2.5 py-2 text-white/80 hover:text-white font-medium text-sm rounded-md hover:bg-white/10 transition-colors">
                {item.label}
              </Link>
            ) : (
              <button
            ...
            {/* Mega Dropdown — Black bg, yellow top border, red hover */}
            {item.columns && activeDropdown === i && (
              <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 bg-[#000000]/95 border-t-[3px] border-[#F5A623] shadow-2xl z-50 overflow-hidden rounded-b-md">
                <div className="flex p-6 gap-8 min-w-[600px] max-w-[720px]">
                  {item.columns.map((col) => (
                    <div key={col.title} className="flex-1 min-w-0">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-[#F5A623] mb-3">{col.title}</h4>
                      <ul className="space-y-1">
                        {col.items.map((sub) => (
                          <li key={sub.label}>
                            <Link href={sub.href}
                              className="block text-sm text-white/70 hover:text-white hover:bg-[#C41E3A] px-2 py-1.5 rounded-sm transition-colors"
      ...
      {/* Mobile Burger */}
      <button className="md:hidden flex flex-col gap-1 p-1.5 -mr-1"
        onClick={() => (mobileOpen ? closeMobile() : openMobile())} aria-label="Menu">
        <span className={`block w-5 h-0.5 bg-white rounded-full transition-all ${mobileOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
        <span className={`block w-4 h-0.5 bg-white rounded-full transition-all ${mobileOpen ? 'opacity-0' : ''}`} />
        <span className={`block w-5 h-0.5 bg-white rounded-full transition-all ${mobileOpen ? '-rotate-45 -translate-y-1.5' : ''}`} />
      </button>
                ...
                <div className="flex-1 overflow-y-auto py-3 px-5">
                  {menuItems[mobileSub].columns?.map((col) => (
                    <div key={col.title} className="mb-5">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-[#C41E3A] mb-2">{col.title}</h4>
                      <ul className="space-y-1">
                        {col.items.map((sub) => (
                          <li key={sub.label}>
                            <Link href={sub.href} className="block py-2 text-sm text-gray-700 border-b border-gray-50" onClick={closeMobile}>{sub.label}</Link>
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto py-2">
                  {menuItems.map((item, i) => (
                    <div key={item.label}>
                      {item.href ? (
                        <Link href={item.href} className="flex items-center justify-between px-5 py-3 text-sm font-medium text-gray-800 border-b border-gray-50" onClick={closeMobile}>{item.label}</Link>
                      ) : (
                        <button className="flex items-center justify-between w-full px-5 py-3 text-sm font-medium text-gray-800 border-b border-gray-50" onClick={() => openSub(i)}>
                          {item.label}
