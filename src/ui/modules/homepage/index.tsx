  ...
  return <div className="max-w-[1180px] mx-auto px-0">
    ...
    <section className="grid lg:grid-cols-[1fr_300px_300px] gap-0.5 py-0.5">
      ...
      <div className="border border-gray-200 p-2">
        ...
        {p.slice(5,19).map((a,i) => (
          <div key={a._id} className="flex items-start gap-1.5 py-[3px] border-b border-gray-100">
            <span className="text-[11px] font-bold text-gray-400 w-4">{i+1}</span>
            <div className="min-w-0"><Link href={`${bp}${a.slug}`} className="text-[11px] text-gray-800 leading-snug line-clamp-2 hover:text-[#C41E3A] transition-colors">{a.title}</Link><Tim p={a} /></div>
          </div>
        ))}
      </div>
    </section>
    ...
    <div className="grid lg:grid-cols-[1fr_300px] gap-0.5 py-0.5">
      ...
      <aside className="border border-gray-200 p-2">
        ...
        {p.slice(6,12).map((a,i) => (<div key={a._id} className="flex items-start gap-1.5 py-[3px] border-b border-gray-100 last:border-b-0"><span className="font-serif text-[24px] font-bold text-[#C41E3A]/20 leading-none">{i+1}</span><div className="min-w-0"><Tag p={a} sz="text-[9px]" /><Link href={`${bp}${a.slug}`} className="text-[12px] leading-snug font-serif text-gray-800 hover:text-[#C41E3A] transition-colors line-clamp-2">{a.title}</Link></div></div>))}
      </aside>
    </div>
    ...
    <section className="border border-gray-200 p-2">
      <h2 className="font-serif text-[14px] font-bold text-gray-800 mb-1">Topik Trending</h2>
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-1">
        {cats.slice(0,8).map(c => { const arts = posts.filter(a => a.cat?.title === c.title).slice(0,4); if (!arts.length) return null; return <div key={c._id} className="border border-gray-100 p-1.5">{arts[0]?.img ? <Link href={`${bp}${arts[0].slug}`} className="block overflow-hidden mb-0.5"><Img p={arts[0]} w={293} h={165} /></Link> : null}<Link href={`${bp}?category=${c.slug}`} className="font-serif text-[13px] font-bold text-[#111] hover:text-[#C41E3A] transition-colors">{c.title}</Link><div className="mt-0.5 divide-y divide-gray-100">{arts.slice(0,3).map(a => (<Link key={a._id} href={`${bp}${a.slug}`} className="block py-0.5 text-[10px] text-gray-600 hover:text-[#C41E3A] transition-colors leading-snug">{a.title}</Link>))}</div></div>})}
      </div>
    </section>
    ...
    <section className="border border-gray-200 p-2">
      ...
      <div className="grid md:grid-cols-3 gap-1">{p.slice(26,29).map(a => (<article key={a._id} className="border border-gray-100 p-1.5"><Link href={`${bp}${a.slug}`} className="block overflow-hidden mb-0.5"><Img p={a} w={380} h={214} /></Link><Tag p={a} sz="text-[10px]" /><Head p={a} sz="text-[12px]" /></article>))}</div>
    </section>

    {/* CTA */}
    <section className="border border-gray-200 p-3 text-center mt-0.5">
